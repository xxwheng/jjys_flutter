...
- (instancetype)initWithTitle:(NSString *)title icon:(UIImage *)icon message:(NSString *)message delegate:(id<WGBAlertViewDelegate>)delegate buttonTitles:(NSString *)buttonTitles, ... {
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        _icon = icon;
        _title = title;
        _message = message;
...
- (void)setContentView:(UIView *)contentView {
    ...
    _contentView.center = CGPointMake(self.center.x, self.center.y-self.centerOffset);
...
- (void)initContentView {
    ...
    _contentView.center = CGPointMake(self.center.x, self.center.y-100);
    [self addSubview:_contentView];
}
...
- (void)show {
	...
	[window.layer removeAnimationForKey:@"an"];
	[window.layer removeAnimationForKey:@"show"];
...
- (void)setTouchBgDismiss:(BOOL)touchBgDismiss{
    _touchBgDismiss = touchBgDismiss;
    if (touchBgDismiss) {
        self.backgroundView.userInteractionEnabled = YES;
        @weakify(self);
        [self.backgroundView addTapActionWithBlock:^(UIGestureRecognizer *gestureRecoginzer) {
            @strongify(self);
            [self hide];
        }];
    }
...
- (void)showBackground
{
	...
	[self.backgroundView.layer addAnimation:animation forKey:@"an"];
...
-(void)showAlertAnimation
{
	...
	[self.contentView.layer addAnimation:animation forKey:@"show"];
